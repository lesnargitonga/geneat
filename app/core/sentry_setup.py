...
def init_sentry(s: Settings) -> bool:
    ...
    try:
        ...
        log.warning("sentry_disabled", reason="sentry_sdk_not_installed")
        return False
    ...
    sentry_sdk.init(
        ...
        send_default_pii=bool(getattr(s, "sentry_send_pii", False)),
        integrations=[
            FastApiIntegration(),
            AsyncioIntegration(),
        ...
        before_send=_scrub if not getattr(s, "sentry_send_pii", False) else None,
    )
    log.info("sentry_enabled", env=s.app_env)
    return True
